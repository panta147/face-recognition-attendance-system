    ...
    try:
        ...
        while True:
            ...
            if face_croped(myframe) is not None:
                ...
                cv2.putText(face, str(imag_id), (50, 50),
...
def Attandance_IN(request):
    def draw_boundary(img, classifier, scalefactor, minneighbors, color, text, clf):
        ...
        for (x, y, w, h) in features:
            ...
            if confidence > 77:
                cv2.putText(
                    img, f'Roll:  {id}', (x, y-55), cv2.FONT_HERSHEY_COMPLEX, 0.8, (10, 223, 242), 3)
                cv2.putText(img, f'Username:  {username}', (x, y-30),
                            cv2.FONT_HERSHEY_COMPLEX, 0.8, (10, 223, 242), 3)
            else:
                cv2.rectangle(img, (x, y), (x+w, y+h), (0, 0, 255), 3)
                cv2.putText(img, 'Unknow face', (x, y-30),
                            cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 255, 255), 3)
            coord = [x, y, w, h]
        return coord

    def recognize(img, clf, faceCascade):
        coord = draw_boundary(img, faceCascade, 1.1, 10,
                              (255, 255, 155), 'Face', clf)
        return img
